 ...
 * @version 		$Id: process.class.php 6791 2013-10-16 07:09:00Z Fern $
 */
		    ...
		    foreach($aSizes as $iSize)
		    {
				...
				if (file_exists($sFile) || Phpfox::getParam('core.allow_cdn'))
				{
					if (Phpfox::getParam('core.allow_cdn') && $aPhoto['server_id'] > 0)
					{
						...
						$sFile = Phpfox::getParam('photo.dir_photo') . $aParts[0] . '/' . $aParts[1] . '/' . md5($aPhoto['destination']) . (empty($iSize) ? '' : '_') . $iSize . '.' . $sExt;
				...
				if (Phpfox::getParam('core.allow_cdn') && $aPhoto['server_id'] > 0)
				{
					$this->database()->update(Phpfox::getT('photo'), array('destination' => $aParts[0] . '/' . $aParts[1] . '/' . md5($aPhoto['destination']) . '%s.' . $sExt), 'photo_id = ' . (int) $aPhoto['photo_id']);
				}
		    }
